...
.seat-checkbox:disabled + label {
    ...
    cursor: not-allowed;
    pointer-events: none; /* Prevents any interaction with the label */
}

/* Disable hover effects on unavailable seats */
.seat-checkbox:disabled + label:hover,
.seat-checkbox:disabled + label li:hover {
    box-shadow: none;
    background-color: red; /* Keeps background red without changes */
}

/* Additional styling for disabled seats inside li and label */
.seat-checkbox:disabled + label li,
.seat-checkbox:disabled + label {
    background-color: red; /* Ensure li and label are also red */
